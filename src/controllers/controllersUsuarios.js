        ...
        if (!nome || !senha || ! confirmSenha) {
            ...
            if (novaSenha.length < 6) {
                ...
                if (novaSenha != novaConfirmSenha) {
                    ...
                    const verificaUsuarioExiste = await pool.query('SELECT * FROM usuario WHERE nome = $1', [novoUsuario]);
                    if (verificaUsuarioExiste.rows.length > 0) {
                      return res.status(200).json({ Mensagem: 'Usuário já existe', status:400 });
                        ...
                        const CadastroUsuario = await pool.query("INSERT INTO usuario (nome, senha) VALUES ($1, $2)", [novoUsuario, senhaCriptografada])
                        if (!CadastroUsuario) {
...
const Login = async (req, res) => {
    try {
        const {
          ...
          const verificaUsuario = await pool.query("SELECT * FROM usuario WHERE nome = $1", [novoUsuario]);
          ...
          const token = jwt.sign({ usuario: verificaUsuario.rows[0].nome }, '802c6ed36c616ef9df379ef94c380f', { expiresIn: 86400 });
...
export {
    MostrarTodosUsuarios, EncontrarUsuarioId, CadastrarUsuario, Login, removeUsuarioID
}
